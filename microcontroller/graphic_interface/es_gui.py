...
# /-------------- CONFIG ------------------------------------/
    ...
    root.mode_var = tk.IntVar(value=1) # 1 = BUCK, 0 = BOOST
...
# /--- WIDGET LAYOUT (All widgets created first) ---/
    ...
    ttk.Label(conn_frame, text = "Puertos de conexión ", font=("Inter", 10, "bold")).pack(side='left',padx=(0, 5))
    
    available_ports = get_ports()
    root.com_port_var = tk.StringVar(value=available_ports[0])
    
    root.com_port_combo = ttk.Combobox(
            conn_frame,
            textvariable=root.com_port_var,
            value=available_ports,
    ...
    # --- Baud Rate Dropdown ---
    ttk.Label(conn_frame, text="Baud rate: ", font=("Inter", 10, "bold")).pack(side='left', padx=(0,5))
    baud_rate_var = tk.StringVar(value="115200")
    baud_frame = ttk.Combobox(
        conn_frame,
        textvariable = baud_rate_var,
        value = ["9600", "19200", "57600", "115200"],
        width=8,
        state='readonly'
    )
    ...
    connect_button = ttk.Button(conn_frame, text="Conectar", style="TButton")
    connect_button.pack(side='left',padx=5)
    
    disconnect_button = ttk.Button(conn_frame, text="Desconectar", style="Danger.TButton", state='disabled')
    ...
    mode_buck = ttk.Button(
        mode_card, text = "BUCK", style = "Primary.TButton"
    )
    mode_boost = ttk.Button(
        mode_card, text = "BOOST", style = "TButton" # Set to default
    )
    ...
    canvas = tk.Canvas(
        wave_label, bg="#e0e0e0", highlightthickness=0
    )
...
# /--- ALL HELPER FUNCTIONS (Defined after widgets) ---/
    ...
    def warning_popup(title, message):
        ...
        if root.mode_var.get() == 1:
            warning_log("Mantenga el Duty bajo 70%")
        elif root.mode_var.get() == 0:
            warning_log("Mantenga el Duty bajo 60%")
        def back_command():
            popup.destroy()
            if root.mode_var.get() == 1:
                duty_var.set(50.0)
    ...
    def draw_pwm_waveform(duty_cycle_percent):
        ...
        canvas.create_rectangle(
            width - 210, 10, width - 10, 40, # Made larger
            fill="#e8db2a", outline=""
        )
        canvas.create_text(
            width - 15, 17, text=v_avg_text, # Centered
            anchor="ne", font=("Inter", 15, "bold"), fill="#000"
        )
        ...
        points = [
            (padding, y_low), (padding, y_high),
            (padding + on_width, y_high), (padding + on_width, y_low),
    ...
    def duty_change(value_str):
        ...
        if root.mode_var.get() == 1: # BUCK
            if rounded_value > 70:
                if (warn_var.get() == 0):
                    warn_var.set(1)
                    warning_popup("Advertencia","Sobre el punto de operacíon sugerido")
                    if duty_var.get() != rounded_value: return
        ...
        elif root.mode_var.get() == 0: # BOOST
            if rounded_value > 60:
                if warn_var.get() == 0:
                    warn_var.set(1)
    ...
    def connect():
        ...
        if "No ports found" in port_full_name:
            warning_log("Conexción Fallida: No hay puerto seleccionado.")
            return
        ...
        try:
            root.serial_connection = serial.Serial(port_name, selected_baud, timeout=1)
